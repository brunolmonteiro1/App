...
export function buildMarkdown(d: ReportData): string {
  ...
  const lines: string[] = [
    ...
    ``,
    `## Sinalizador lexical de crítica religiosa por ano (métrica lexical)`,
    ``,
    `| Ano | Sinal médio | n |`,
    `|---|---|---|`,
    ...d.iscByYear.map((r) => `| ${r.year} | ${r.isc}% | ${r.n} |`),
    ``,
    `_Sinal = menções de crítica ao sistema / menções do Evangelho × 100. Vocabulário lexical preliminar — não mede intenção, tom nem fundamentação._`,
    ``,
